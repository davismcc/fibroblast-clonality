...
Here, we will lok at differential expresion between clones across all donors at
the gene and gene set levels.
...
ggplot(aes(x = n_donors, y = count)) +
    ...
    theme_classic(20) +
    xlab("Number of donors in which significant (FDR < 10%)")  +
...
ggplot(aes(x = n_donors, y = count)) +
    ...
    theme_classic(16) +
    xlab("Number of donors significant (FDR < 10%)")  +
    ylab("Number of genes")
p1 
...
p2 <- df_de_sig_unst %>% 
    ...
    coord_flip() +
    theme_bw(16) +
    xlab("Gene") + ylab("Number of donors significant")
p2
...
df_gene_n_de %>% dplyr::filter(count >= 8) %>% print(n = Inf)
```
...
```{r alldonors_camera_enrichment_by_donor, fig.height=12, fig.width=8}
df_camera_all_unst %>% 
    dplyr::filter(stat == "logFC") %>%
    group_by(donor, msigdb_collection, contrast) %>%
    summarise(n_sig = sum(FDR < 0.05)) %>% ungroup() %>% 
    dplyr::mutate(donor = factor(donor, levels = rev(levels(factor(donor)))),
                  pos_selection = as.character(donor)  %in% pos_sel_donors) %>%
    ggplot(aes(y = n_sig, x = donor,
               fill = contrast, shape = pos_selection)) +
    geom_point(alpha = 1, size = 4, colour = "gray50") +
    facet_wrap(~ msigdb_collection, scales = "free_x") + 
    scale_shape_manual(values = c(21, 23), name = "positive\nselection") +
    scale_fill_brewer(palette = "Accent") +
    coord_flip() +
    theme_bw(16) +
    xlab("Donor") + ylab("Number of significant genesets (FDR < 5%)") +
    ggtitle("Camera MSigDB gene set enrichment by donor") +
    guides(fill = guide_legend(override.aes = list(shape = 21)))


ggsave("figures/differential_expression/alldonors_camera_enrichment_by_donor.png", 
       height = 7, width = 10)
ggsave("figures/differential_expression/alldonors_camera_enrichment_by_donor.pdf", 
       height = 7, width = 10)
ggsave("figures/differential_expression/alldonors_camera_enrichment_by_donor.svg", 
       height = 7, width = 10)
```

A slightly simpler version of that plot.
...
df_camera_all_unst %>% 
    ...
    dplyr::mutate(donor = factor(donor, levels = rev(levels(factor(donor)))),
                  pos_selection = as.character(donor)  %in% pos_sel_donors) %>%
    ggplot(aes(y = n_sig, x = donor, shape = pos_selection)) +
    geom_point(alpha = 1, size = 4) +
    facet_wrap(~ msigdb_collection, scales = "free_x") + 
    scale_shape_manual(values = c(1, 8), name = "positive\nselection") +
    scale_fill_brewer(palette = "Accent") +
    coord_flip() +
    theme_bw(16) +
    xlab("Donor") + ylab("Number of significant genesets (FDR < 5%)") +
...
df_to_plot %>%
    dplyr::mutate(donor = factor(donor, levels = rev(levels(factor(donor)))),
                  pos_selection = as.character(donor)  %in% pos_sel_donors) %>%
    ggplot(aes(y = n_sig, x = donor, shape = pos_selection, size = ncells)) +
    geom_point(alpha = 1) +
    facet_wrap(~ msigdb_collection, scales = "free_x") + 
    scale_shape_manual(values = c(1, 8), name = "positive\nselection") +
    scale_fill_brewer(palette = "Accent") +
...
ggplot(aes(y = n_donors, x = reorder(geneset, n_donors, max))) +
    ...
    coord_flip() +
    theme_bw(14) +
    xlab("Gene set") + ylab("Number of donors significant")
...
```{r alldonors_camera_enrichment_H_by_geneset_by_dir, fig.height=7, fig.width=10}
df_camera_sig_unst %>% dplyr::filter(collection == "H") %>% 
  dplyr::filter(stat == "logFC") %>%
...
pp <- df_4_heatmap_all %>%
    ...
    ylab("Gene set") +
    xlab("Donor and clone comparison") +
    theme(axis.text.x = element_text(angle = 60, hjust = 1),
...
Finally, we can add a panel to this figure showing the number of donors in which
...
pp_nsig <- df_camera_sig_unst %>% dplyr::filter(collection == "H") %>% 
    ...
    ggthemes::scale_colour_tableau() +
    coord_flip() +
    xlab("Gene set") + ylab("Number of donors\nsignificant") +
    theme(axis.title.y = element_blank(),
          axis.text.y = element_blank(),
          axis.ticks.y = element_blank(),
          axis.line.y = element_blank())

plot_grid(corrplot1 + theme(legend.position = "top"), 
          pp_nsig, rel_widths = c(7, 3), rel_heights = c(1, 0.1))
