...
```{r donor-mutation-selection, fig.height=12, fig.width=12}
...
```{r donor-mutation-signatures-cosmic, fig.height=3.5, fig.width=6}
data("cosmic_signatures", package = "sigfit")

##new input 
mutation_list <- read.table("data/high-vs-low-exomes.v62.ft.filt_lenient-alldonors.txt.gz", header = TRUE)
...
canopy_list <- list()
for (don in donors) {
        cat(paste("reading canopy data for", don, "\n"))
...
Next, summarise the number of unique mutations tagging each clone identified by
...
```{r plot-cell-assignment-vs-hamming, fig.height=12, fig.width=12}
