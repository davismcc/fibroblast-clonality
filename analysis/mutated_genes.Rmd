...
relevant donor. Assignment of cells to clones was done with [cardelino](https://github.com/PMBio/cardelino).
...
There are `r nrow(vep_best)` variants, aggregated across all donors, and we can 
...
`r length(donors)` donors, with `r round(100 * mean(assignments$assigned != "unassigned"))`% 
...
filtered set of affected genes for each donor.
...
dplyr::arrange(mut_genes_df_allcells_filt, FDR) %>% 
  dplyr::select(-location, donor, hgnc_symbol, var_id, consequence, FDR, PValue,
                everything()) %>% 
  DT::datatable(., options = list(pageLength = 20))
```
...
categories and assigned to combinations of clones, aggregated across all donors.
...
each annotation category for each donor separately.
...
There are only `r sum(df_genes_multi_vars$n_variants_in_gene > 1.5 & df_genes_multi_vars$max_dist_btw_vars > 1.5)` genes, across all donors, that 
...
mut_genes_df_allcells %>%
  dplyr::filter(donor %in% df_genes_multi_vars[["donor"]][idx], 
...
df_cc_vars <- mut_genes_df_allcells %>% 
  ...
  group_by(donor, consequence_simplified) %>%
  summarise(n_vars = n())
df_cc_vars <- left_join(
  expand.grid(donor = levels(as.factor(mut_genes_df_allcells$donor)),
...
ggplot(df_cc_vars, 
  ...
  ggtitle("Variants in proliferation genes: all donors") +
  xlab("number of variants") + ylab("consequence") +
  theme_bw(16)
```
...
ggplot(df_prolif_vars, 
  ...
  scale_fill_manual(values = colorRampPalette(brewer.pal(8, "Accent"))(18)) +
  guides(colour = FALSE, fill = FALSE) +
  ggtitle("Variants in cell cycle genes: all donors") +
  xlab("number of variants") + ylab("consequence") +
  theme_bw(16)
```
...
```{r}
colnames(df_cc_vars) <- c("donor", "consequence", "n_vars_cellcycle_genes")
colnames(df_prolif_vars) <- c("donor", "consequence", "n_vars_proliferation_genes")
...
colnames(df_all_vars) <- c("donor", "consequence", "n_vars_all_genes")

df_nvars <- inner_join(inner_join(df_all_vars, df_cc_vars), df_prolif_vars) %>%
  as_data_frame

readr::write_tsv(df_nvars, "output/nvars_by_category_by_donor.tsv")

df_nvars %>% 
  DT::datatable(., options = list(pageLength = 20))
```

We can also look at the total mutational load for each of the donors.
...
set of variants and donors (with relatively small sample size in terms of 
