...
# Differential gene and pathway expression analysis between base and other clones

**Cardelino cell assignments using Canopy trees**.

** Comparing base clone to other clones**.

Using the latest callset from Petr Danecek (April 2018): fisher exact test and other filters.
...
```{r}
donors <- c('bubh', 'ciwj', 'deyz', 'diku', 'eipl', 'eofe', 'euts', 'fawm', 'feec', 'fiaj', 'fikt', 
            'garx', 'gesg', 'gifk', 'hehd', 'heja', 'hipn', 'ieki', 'jogf', 'joxm', 'laey', 'lexy', 
            'melw', 'miaj', 'naju', 'nusw', 'oaaz', 'oaqd', 'oicx', 'oilg', 'pamv', 'pelm', 'pipw', 
            'puie', 'qayj', 'qolg', 'qonc', 'rozh', 'sebz', 'sehl', 'sohd', 'tixi', 'ualf', 'vabj', 
            'vass', 'vils', 'vuna', 'wahn', 'wetu', 'wuye', 'xugn', 'zihe', 'zoxy')
length(donors)
```

```{r}
## Read in annotated SCE objects
sce_list <- list()
sce_unst_list <- list()
for (don in donors) {
    sce_list[[don]] <- readRDS(file.path("../../Data/processed/sces", paste0("sce_", don, "_with_clone_assignments.rds")))
    sce_unst_list[[don]] <- sce_list[[don]][, sce_list[[don]]$well_condition == "unstimulated"]
    cat(paste("reading", don, ":   ", ncol(sce_list[[don]]), "cells total,", ncol(sce_unst_list[[don]]), "cells unstimulated.\n"))
}
```

```{r}
length(sce_unst_list)
```

Define each cell as either "base clone", "other clone" or "unassigned"

```{r}
tmp_base <- ifelse(sce_list[[don]]$base_clone, "base_clone", "other_clone")
tmp_base[is.na(tmp_base)] <- "unassigned"
table(tmp_base)
```
...
for (don in donors) {
    tmp_base <- ifelse(sce_list[[don]]$base_clone, "base_clone", "other_clone")
    tmp_base[is.na(tmp_base)] <- "unassigned"
    sce_list[[don]]$base_or_other <- tmp_base
    tmp_base <- ifelse(sce_unst_list[[don]]$base_clone, "base_clone", "other_clone")
    tmp_base[is.na(tmp_base)] <- "unassigned"
    sce_unst_list[[don]]$base_or_other <- tmp_base
}
```
...
```{r}
table(sce_unst_list[["laey"]]$base_clone)
```

```{r}
table(sce_unst_list[["qayj"]]$base_or_other)
```

```{r}
table(sce_unst_list[["qayj"]]$clone_apk2)
```
...
for (i in names(sce_unst_list)) {
    if (ncol(sce_unst_list[[i]]) == 0)
        ...
        tmp <- as_data_frame(table(sce_unst_list[[i]]$base_or_other))
...
for (i in names(sce_list)) {
    if (ncol(sce_list[[i]]) == 0)
        ...
        tmp <- as_data_frame(table(sce_list[[i]]$base_or_other))
        colnames(tmp) <- c("clone", "n_cells")
        tmp[["donor"]] <- i
        tmp2 <- data.frame(clone = "total assigned", 
                           n_cells = sum(tmp[["n_cells"]][tmp[["clone"]] != "unassigned"]), donor = i)
...
ggplot(df_cells_per_clone_unst, aes(x = n_cells, y = reorder(donor, n_cells, max), colour = clone, shape = assigned)) +
geom_point(size = 4, alpha = 0.7) +
scale_shape_manual(values = c(4, 19)) +
scale_color_manual(values = c(magma(4)[-c(1,4)], "gray30", "black")) +
...
geom_point(size = 4, alpha = 0.7) +
scale_shape_manual(values = c(4, 19)) +
scale_color_manual(values = c(magma(4)[-c(1,4)], "black")) +
#ggthemes::scale_colour_tableau() +
theme_bw(16) + xlab("Number of cells") + ylab("Donor") +
ggtitle("Unstimulated cells per clone, by donor")
```
...
clones_keep <- list()
for ( i in names(sce_list) ) {
    clones_keep[[i]] <- table(sce_list[[i]]$base_or_other)[which(table(sce_list[[i]]$base_or_other) >= 3)]
    clones_keep[[i]] <- clones_keep[[i]][!(names(clones_keep[[i]]) %in% "unassigned")]
    if (length(clones_keep[[i]]) < 1.5) {
        cat(paste("....", i, ":", "only one clone with cells assigned :( \n"))
        next
    }
    cells_use <- colnames(sce_list[[i]])[sce_list[[i]]$base_or_other %in% names(clones_keep[[i]])]
...
25 donors are retained for DE/pathway analyses from all cells.
...
for ( i in names(sce_unst_list) ) {
    clones_keep_unst[[i]] <- table(sce_unst_list[[i]]$base_or_other)[which(table(sce_unst_list[[i]]$base_or_other) >= 3)]
    clones_keep_unst[[i]] <- clones_keep_unst[[i]][!(names(clones_keep_unst[[i]]) %in% "unassigned")]
    if (length(clones_keep_unst[[i]]) < 1.5) {
        cat(paste("....", i, ":", "only one clone with cells assigned :( \n"))
        next
    }
    cells_use_unst <- colnames(sce_unst_list[[i]])[sce_unst_list[[i]]$base_or_other %in% names(clones_keep_unst[[i]])]
...
20 donors are retained for DE/pathway anlaysis using unstimulated cells only.
...
```{r}
data_dir <- "../../Data/processed/de_analysis_FTv62/"
```
...
for(i in names(sce_unst_list_filt)) {
  #sce_list_unst[[i]] <- scran::computeSumFactors(sce_list_unst[[i]])
    ...
    design_list_unst[[i]] <- model.matrix(~cdr + plate + base_or_other, data = colData(sce_unst_list_filt[[i]]))
    ...
    qlf_list_unst[[i]] <- glmQLFTest(fit_list_unst[[i]], coef = ncol(design_list_unst[[i]]))
    sum(p.adjust(qlf_list_unst[[i]]$table$PValue, method = "BH") <= 0.05, na.rm = TRUE)
}
```

Conduct QL F-test for first clone coefficient alone (other_clone vs base_clone).
...
for(i in names(sce_unst_list_filt)) {
    ...
    camera_msigdb_c6_1st_coef_list_unst[[i]] <- camera(dge_list_unst[[i]], idx, qlf_list_unst[[i]]$design, 
                                                  contrast = ncol(design_list_unst[[i]]))
...
```{r}
saveRDS(object = de_results_list_unst, file = file.path(data_dir, "de_results_base-vs-other_unstimulated_cells.rds"))
```
...
for(i in names(sce_unst_list_filt)) {
  ...
  png(file.path(data_dir, paste0("BCV_plots/BCV_", i, ".png")))
  plotBCV(dge_list_unst[[i]])
  dev.off()
  png(file.path(data_dir, paste0("MD_plots/MD_", i, ".png")))
...
df_camera_sig_unst <- dplyr::mutate(df_camera_sig_unst,
                              contrast = plyr::mapvalues(coeff, from = c("1st_coef"), to = c("other clones - base clone")),
...
ggplot(aes(y = n_sig, x = reorder(donor, n_sig, max))) +
    ...
    theme_bw(16) +
    xlab("Donor") + ylab("Number of significant genesets") +
    ggtitle("Camera gene set enrichment results (other clones vs base clone; unstimulated cells)")
```

```{r}
df_camera_sig_unst %>% dplyr::filter(donor == "eofe", collection == "H") 
```
...
```{r}
table(de_results_list_unst[["sce_list_unst"]][["sohd"]]$base_or_other)
```

```{r}
table(de_results_list_unst[["sce_list_unst"]][["sehl"]]$base_or_other)
```

```{r}
table(de_results_list_unst[["sce_list_unst"]][["eofe"]]$base_or_other)
```

```{r}
table(de_results_list_unst[["sce_list_unst"]][["laey"]]$base_or_other)
```

```{r}
options(repr.plot.width = 9, repr.plot.height = 5)
idx <- ids2indices(Hs.H, id=de_results_list_unst$qlf_1st_coef_list$eofe$table$entrezid)
barcodeplot(de_results_list_unst$qlf_1st_coef_list$eofe$table$logFC, index=idx$HALLMARK_E2F_TARGETS, 
            index2=idx$HALLMARK_G2M_CHECKPOINT, xlab = "logFC", main="eofe: Other clones - Base clone\n HALLMARK_E2F_TARGETS and HALLMARK_G2M_CHECKPOINT")
```
...
```{r}
options(repr.plot.width = 9, repr.plot.height = 5)
idx <- ids2indices(Hs.H, id=de_results_list_unst$qlf_1st_coef_list$sehl$table$entrezid)
barcodeplot(de_results_list_unst$qlf_1st_coef_list$sehl$table$logFC, index=idx$HALLMARK_E2F_TARGETS, 
            index2=idx$HALLMARK_G2M_CHECKPOINT, xlab = "logFC", main="sehl: Other clones - Base clone\n HALLMARK_E2F_TARGETS and HALLMARK_G2M_CHECKPOINT")
```
...
```{r}
options(repr.plot.width = 9, repr.plot.height = 5)
idx <- ids2indices(Hs.H, id=de_results_list_unst$qlf_1st_coef_list$laey$table$entrezid)
barcodeplot(de_results_list_unst$qlf_1st_coef_list$laey$table$logFC, index=idx$HALLMARK_E2F_TARGETS, 
            index2=idx$HALLMARK_G2M_CHECKPOINT, xlab = "logFC", main="laey: Other clones - Base clone\n HALLMARK_E2F_TARGETS and HALLMARK_G2M_CHECKPOINT")
```
...
ggplot(aes(y = n_donors, x = reorder(geneset, n_donors, max))) +
    ...
    theme_bw(16) +
    xlab("Gene set") + ylab("Number of significant donors")  +
    ggtitle("Camera MSigDB Hallmark gene set enrichment", subtitle = "Unstimulated cells; Other clones vs Base clone")
```
...
ggplot(aes(y = n_donors, x = reorder(geneset, n_donors, max), colour = Direction, shape = contrast)) +
    ...
    theme_bw(16) +
    xlab("Gene set") + ylab("Number of significant donors")  +
    ggtitle("Camera MSigDB oncogenic gene set enrichment", subtitle = "Unstimulated cells; Other clones vs Base clone")
```
...
df_camera_sig_unst %>% dplyr::filter(collection == "c2") %>% group_by(geneset, contrast, Direction) %>% 
    summarise(n_donors = n()) %>% dplyr::arrange(geneset, n_donors) %>% ungroup() %>%
    dplyr::filter(geneset %in% geneset[n_donors > 4.5]) %>% dplyr::mutate(geneset_short = substr(geneset, 1, 40)) %>%
ggplot(aes(y = n_donors, x = reorder(geneset_short, n_donors, max), colour = Direction, shape = contrast)) +
    ...
    theme_bw(14) +
    xlab("Gene set") + ylab("Number of significant donors")  +
    ggtitle("Camera MSigDB curated (c2) gene set enrichment", subtitle = "Unstimulated cells; Other clones vs Base clone")
```
...
df_camera_sig_unst %>% dplyr::filter(collection == "c2") %>% group_by(geneset) %>% 
    dplyr::mutate(id = paste0(donor, geneset)) %>% distinct(id, .keep_all = TRUE) %>%
    summarise(n_donors = n()) %>% dplyr::arrange(geneset, n_donors) %>% ungroup() %>%
    dplyr::filter(geneset %in% geneset[n_donors > 7.5]) %>% dplyr::mutate(geneset_short = substr(geneset, 1, 40)) %>%
...
df_de_sig_unst <- dplyr::mutate(df_de_sig_unst,
                              contrast = plyr::mapvalues(coeff, from = c("qlf_list", "qlf_1st_coef_list"), 
                                                         to = c("any difference", "other clones - base clone")),
...
ggplot(aes(y = n_donors, x = reorder(hgnc_symbol, n_donors, max))) +
    geom_segment(aes(x = reorder(hgnc_symbol, n_donors, max), xend = reorder(hgnc_symbol, n_donors, max), y = 0, yend = n_donors),
                alpha = 0.2) +
    geom_point(colour = "gray40", size = 4) +
    ggthemes::scale_colour_tableau() +
    coord_flip() +
    theme_classic(14) + ylim(0, 6) +
    xlab("Gene") + ylab("Number of significant donors")  +
    ggtitle("edgeR QL F-test DE genes (any difference between clones; significant if FDR <5%)", subtitle = "Unstimulated cells; other clones vs base clone")
```
...
ggtitle("Significant genes and genesets by donor", subtitle = "Unstimulated cells; Other clones vs Base clone")
```
...
ggtitle("Number of significant curated (C2) gene sets against number of cells by donor", subtitle = "Unstimulated cells; Other clones vs Base clone")
```
...
scale_fill_viridis(option = "B") +
guides(fill = FALSE) +
ylab("Number of significant Hallmark gene sets (FDR <5%)") + xlab("Number of cells") +
ggtitle("Number of significant Hallmark gene sets against number of cells by donor", subtitle = "Unstimulated cells; Other clones vs Base clone")
```
...
ggtitle("Number of significant DE genes against number of cells by donor", subtitle = "Unstimulated cells; Other clones vs Base clone")
```
...
```{r}
options(repr.plot.width = 9, repr.plot.height = 6)
plotMD(qlf_1st_coef_list_unst[["eofe"]])
```

```{r}
options(repr.plot.width = 9, repr.plot.height = 6)
plotMD(qlf_1st_coef_list_unst[["joxm"]])
```
...
par(mfrow = c(5, ceiling(length(qlf_1st_coef_list_unst[["joxm"]]) / 5 - 0.1)))
for (i in names(qlf_1st_coef_list_unst))
    plotMD(qlf_1st_coef_list_unst[[i]])
```
...
for(i in names(sce_list_filt)) {
  #sce_list_unst[[i]] <- scran::computeSumFactors(sce_list_unst[[i]])
    ...
    design_list_all[[i]] <- model.matrix(~cdr + plate + well_condition + assigned, data = colData(sce_list_filt[[i]]))
...
```{r}
saveRDS(object = de_results_list_unst, file = file.path(data_dir, "de_results_unstimulated_cells.rds"))
```
